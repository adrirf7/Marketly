  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-30">
      ...
      <div className="border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-0.5 xs:px-1 sm:px-4 py-1 xs:py-1.5 sm:py-3">
          <div className="flex items-center gap-0 xs:gap-0.5 sm:gap-2 md:gap-4">
            <a href="/" className="flex-shrink-0 scale-75 xs:scale-90 sm:scale-100 origin-left">
              <MarketlyLogo />
            </a>
            ...
            <div className="flex items-center gap-0 xs:gap-0.5 sm:gap-2 md:gap-3 ml-auto flex-shrink-0">
              {/* Botón de búsqueda móvil */}
              <button
                className="md:hidden p-0.5 xs:p-1 sm:p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0"
                onClick={() => setShowSearchMobile(!showSearchMobile)}
                aria-label="Buscar"
              >
                <svg className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                className="relative p-0.5 xs:p-1 sm:p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0"
                onClick={onCartClick}
                aria-label="Abrir carrito"
              >
                <CartIcon />
                {itemCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 bg-red-600 text-white text-[8px] xs:text-[10px] sm:text-xs font-bold rounded-full w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 flex items-center justify-center shadow-lg">
                    {itemCount}
                  </span>
                )}
              </button>
      ...
      <div className="border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-0.5 xs:px-1 sm:px-4 py-0.5 xs:py-1 sm:py-2 md:py-3">
          <div className="flex items-center gap-0 xs:gap-0.5 sm:gap-2 md:gap-4">
            <CategoryDropdown />
            <FilterDropdown />
